...
import { updateChantier, deleteChantier } from "@/server/planning/actions";
...
export async function PATCH(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return withWriteErrorHandling(async () => {
    ...
    const result = await updateChantier(String(body.crmId), id, jsonToFormData(body), access.actor);
...
export async function DELETE(request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  return withWriteErrorHandling(async () => {
    ...
    const result = await deleteChantier(crmId, id, access.actor);
    return actionResultResponse(result);
  });
}
