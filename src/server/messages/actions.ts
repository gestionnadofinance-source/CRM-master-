...
const directThreadSchema = z.object({ otherUserId: z.string().trim().max(MAX_ID, tooLong(MAX_ID)).regex(NO_CONTROL_CHARS, CONTROL_CHARS_MESSAGE).min(1) });
...
const groupThreadSchema = z.object({
  ...
  participantIds: z.array(z.string().trim().max(MAX_ID, tooLong(MAX_ID)).regex(NO_CONTROL_CHARS, CONTROL_CHARS_MESSAGE).min(1)).min(1, "Sélectionnez au moins un participant."),
});
