...
const companySettingsSchema = z.object({
  ...
  ape: z.string().trim(),
  urssafOffice: z.string().trim(),
  legalRepresentative: z.string().trim(),
  missionOrderLegalMentions: z.string().trim(),
});
...
const sourceSchema = z.object({ name: z.string().trim().min(1, "Le nom est obligatoire."), order: z.coerce.number().int() });
...
const customFieldSchema = z.object({
  ...
  options: z.array(z.string().trim()).transform((arr) => arr.filter(Boolean)),
  required: z.boolean(),
  order: z.coerce.number().int(),
});
